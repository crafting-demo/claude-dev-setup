...
2. **⚙️ Set environment variables** in your sandbox with `ANTHROPIC_API_KEY` secret access
...
Options:
  ...
  🛠️ -t, --tools <value>            Tool whitelist string or file path (optional)
  📐 -template, --template <value>  Custom template name (default: claude-code-automation)
  🗑️ -d, --delete-when-done <yes|no> Delete sandbox when done (default: yes)
  🏷️ -n, --name <name>              Sandbox name (default: auto-generated)
...
2. **⚙️ Set environment variables** - Ensure `ANTHROPIC_API_KEY` is configured as a Crafting secret path in your sandbox environment
