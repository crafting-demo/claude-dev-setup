...
func main() {
	...
	rootCmd.Flags().StringVarP(&opts.mcpCfg, "mcp-config", "mc", "", "External MCP config JSON string or file path")
	rootCmd.Flags().StringVarP(&opts.agentsDir, "agents-dir", "ad", "", "Directory containing agent .md files")
	...
	rootCmd.Flags().StringVarP(&opts.taskID, "task-id", "tid", "", "Custom task ID")
	rootCmd.Flags().StringVar(&opts.debug, "debug", "no", "Debug mode: yes|no")
	rootCmd.Flags().StringVarP(&opts.customRepoPath, "repo-path", "rp", "", "Custom repo path inside sandbox (optional)")
